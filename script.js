...
const SECRET_ASSET_CATEGORIES = [
  ...
  { category: 'realEstate', label: 'Недвижимость', duplicates: 1 },
  { category: 'construction', label: 'Строительство', duplicates: 1 },
  { category: 'securities', label: 'Ценные бумаги', duplicates: 1 },
  { category: 'business', label: 'Участие в бизнесе', duplicates: 1 },
  { category: 'inventory', label: 'Закупка товара', duplicates: 2, includeOnRounds: [2, 3], trackRemaining: true },
  { category: 'land', label: 'Земельные участки', includeOnRounds: [] },
  { category: 'funds', label: 'Паевые фонды', includeOnRounds: [] },
  { category: 'deposits', label: 'Банковские вклады', includeOnRounds: [] },
  { category: 'privateLoans', label: 'Частные займы', includeOnRounds: [] },
  { category: 'gold', label: 'Золотые изделия', includeOnRounds: [] },
  { category: 'other', label: 'Другие активы', includeOnRounds: [] }
];

const SECRET_ASSET_FAMILY_STYLES = {
  property: { accent: '#b47a6d', fill: '#f5ece9' },
  securities: { accent: '#6d7fb2', fill: '#eef1f8' },
  business: { accent: '#4f8a6d', fill: '#eef5f1' },
  savings: { accent: '#7d8391', fill: '#f2f4f6' },
  other: { accent: '#c5a65a', fill: '#f9f4e4' },
  inventory: { accent: '#8a74b1', fill: '#f1ecfa' }
};
...
const SECRET_MARKETING_LABELS = {
  packages: 'Продукт «Бизнес-пакеты»',
  vitamins: 'Продукт «Витамины»'
};
  ...
  SECRET_ASSET_CATEGORIES.forEach(category => {
    ...
    for (let index = 0; index < count; index += 1) {
      ...
      rows.push({
        ...
        trackRemaining: !!category.trackRemaining
      });
    }
  });
  return rows;
...
function getSecretAssetDefinition(round, key) {
  const config = SECRET_ROUND_CONFIG[round];
  if (!config || typeof config.getAssetRows !== 'function') {
    return null;
  }
  const rows = config.getAssetRows();
  return rows.find(item => item.key === key) || null;
}

function getSecretAssetFamily(category) {
  switch (category) {
    case 'land':
    case 'construction':
    case 'realEstate':
...
function createDefaultSecretGoals() {
  return {
    passiveTarget: 2000000,
    passiveAchieved: false,
    dreamAchieved: false,
    victoryShown: false,
...
function createEmptySecretState() {
  return {
    ...
    goals: createDefaultSecretGoals()
  };
}
...
function ensureSecretState(state) {
  ...
  normalized.playerInfo.dreamConfirmed = !!normalized.playerInfo.dreamConfirmed;
  normalized.goals = {
    ...createDefaultSecretGoals(),
    ...(source.goals || {})
  };
  normalized.goals.passiveTarget = Number(normalized.goals.passiveTarget) || 2000000;
  normalized.goals.passiveAchieved = !!normalized.goals.passiveAchieved;
  normalized.goals.dreamAchieved = !!normalized.goals.dreamAchieved;
  normalized.goals.victoryShown = !!normalized.goals.victoryShown;
  normalized.goals.reportUnlocked = !!normalized.goals.reportUnlocked;

  return normalized;
}

let secretEntryCounter = 0;
let secretVictoryTimer = null;
...
function applySecretConfigDefaults(sheet, config) {
  ...
  assetRows.forEach(item => {
    const allowQuantity = item.allowQuantity !== false;
    ...
    existing.quantity = allowQuantity ? Number(existing.quantity ?? 0) : 1;
    ...
    existing.allowQuantity = allowQuantity;
    existing.confirmed = !!existing.confirmed;
    existing.appliedIncome = Number(existing.appliedIncome) || 0;
    existing.trackRemaining = !!item.trackRemaining;
    existing.remaining = Number(existing.remaining) || 0;
    if (!allowQuantity) {
      existing.quantity = 1;
    }
  });
  ...
  sheet.extraAssets = sheet.extraAssets.map(entry => ({
    ...entry,
    price: Number(entry?.price) || 0,
    quantity: Number(entry?.quantity) || 0,
    income: Number(entry?.income) || 0,
    category: entry?.category || 'other',
    label: entry?.label || 'Дополнительный актив',
    allowQuantity: entry?.allowQuantity === false ? false : true,
    confirmed: !!entry?.confirmed,
    appliedIncome: Number(entry?.appliedIncome) || 0,
    trackRemaining: !!entry?.trackRemaining,
  ...
  if (!sheet.marketing || typeof sheet.marketing !== 'object') {
    ...
    sheet.marketing.packages = {
      ...
      multiplier: Math.max(1, Number(sheet.marketing.packages?.multiplier) || 1),
      applied: Number(sheet.marketing.packages?.applied) || 0
    };
    sheet.marketing.vitamins = {
      income: Number(sheet.marketing.vitamins?.income) || 0,
      count: Number(sheet.marketing.vitamins?.count) || 0,
      multiplier: Math.max(1, Number(sheet.marketing.vitamins?.multiplier) || 1),
      applied: Number(sheet.marketing.vitamins?.applied) || 0
    };
  }
...
function inheritSecretAssets(fromRound, toRound, categories = []) {
  const fromSheet = ensureSecretSheet(fromRound);
  const toSheet = ensureSecretSheet(toRound);
  if (!fromSheet || !toSheet) {
    return;
  }

  const copyCategories = categories.length ? categories : ['realEstate', 'land', 'construction', 'securities', 'funds', 'business', 'inventory'];

  Object.entries(fromSheet.assets || {}).forEach(([key, asset]) => {
    if (!asset) return;
    const category = asset.category || key.replace(/_.+$/, '');
    if (!copyCategories.includes(category)) return;
    const base = toSheet.assets[key] || {};
    toSheet.assets[key] = {
      ...base,
      name: asset.name || base.name || '',
      price: Number(asset.price) || 0,
      income: Number(asset.income) || 0,
      quantity: asset.allowQuantity === false ? 1 : Number(asset.quantity) || 0,
      allowQuantity: asset.allowQuantity === false ? false : true,
      confirmed: !!asset.confirmed,
      appliedIncome: Number(asset.appliedIncome) || 0,
      category: category,
      label: asset.label || base.label,
      trackRemaining: !!asset.trackRemaining,
      remaining: Number(asset.remaining) || 0
    };
  });

  const targetExtras = Array.isArray(toSheet.extraAssets) ? toSheet.extraAssets : [];
  const extrasById = new Map(targetExtras.map(entry => [entry.id, entry]));

  (fromSheet.extraAssets || []).forEach(entry => {
    if (!entry) return;
    ...
    const clone = {
      ...
      price: Number(entry.price) || 0,
      income: Number(entry.income) || 0,
      quantity: entry.allowQuantity === false ? 1 : Number(entry.quantity) || 0,
      allowQuantity: entry.allowQuantity === false ? false : true,
      confirmed: !!entry.confirmed,
      appliedIncome: Number(entry.appliedIncome) || 0,
      trackRemaining: !!entry.trackRemaining,
    ...
    if (extrasById.has(entry.id)) {
      Object.assign(extrasById.get(entry.id), clone);
    } else {
      const copy = { ...clone };
      targetExtras.push(copy);
      extrasById.set(entry.id, copy);
    }
  });

  toSheet.extraAssets = targetExtras;
  updateSecretSheetTotals(toRound);
}
...
function selectProfession(index, type) {
  if (type === 'cashflow') {
    ...
    gameType = 'secret';
    const initialized = initializeSecretGame();
    if (initialized) {
      navigateToScreen(7);
    } else {
      showModal(
        'Не удалось открыть «Секрет денег»',
        'Возникла ошибка при подготовке бланка. Попробуйте снова выбрать профессию '
...
function initializeSecretGame() {
  ...
  if (secretVictoryTimer) {
    clearTimeout(secretVictoryTimer);
    secretVictoryTimer = null;
  }

  const assignFreshSecretState = () => {
    const freshState = createEmptySecretState();
    freshState.playerInfo.profession = selectedProfession ? selectedProfession.name : '';
    if (currentUser && currentUser.nickname) {
      freshState.playerInfo.player = currentUser.nickname;
    }
    gameData.secretData = ensureSecretState(freshState);
    ...
    gameData.secretData.goals = createDefaultSecretGoals();
    gameData.secretData.playerInfo.dreamConfirmed = false;
  ...
  assignFreshSecretState();

  try {
    finalizeInitialization();
    return true;
  } catch (error) {
    console.error('Не удалось инициализировать «Секрет денег» с сохранёнными данными:', error);
    showModal(
      'Данные «Секрет денег» сброшены',
      'При восстановлении прежнего листа возникла ошибка, поэтому бланк был создан заново.'
    );
    ...
    try {
      ...
      console.error('Повторная инициализация «Секрет денег» завершилась ошибкой:', fallbackError);
      return false;
    }
  }
}
...
function applySecretProfessionDefaults() {
  ...
  if (sheet1 && sheet1.assets?.salary) {
    ...
    sheet1.assets.salary.quantity = 1;
    sheet1.assets.salary.category = 'salary';
    sheet1.assets.salary.label = 'Заработная плата';
    sheet1.assets.salary.allowQuantity = false;
    sheet1.assets.salary.confirmed = true;
    sheet1.assets.salary.appliedIncome = salary;
  }
...
function clearSecretSheetDom(round) {
  ...
  const packagesMultiplier = document.getElementById(`secretRound${round}PackagesMultiplier`);
  if (packagesMultiplier) packagesMultiplier.textContent = '';
  const vitaminsTotal = document.getElementById(`secretRound${round}VitaminsTotal`);
  if (vitaminsTotal) vitaminsTotal.textContent = '₽0';
  const vitaminsMultiplier = document.getElementById(`secretRound${round}VitaminsMultiplier`);
  if (vitaminsMultiplier) vitaminsMultiplier.textContent = '';
...
function renderSecretSheet(round) {
  ...
  aggregateSecretIncome(round, sheet);
...
function renderSecretAssetsTable(round, sheet, config) {
  const mainBody = document.getElementById(`secretRound${round}Assets`);
  const inventoryBody = document.getElementById(`secretRound${round}Inventory`);
  if (mainBody) {
    mainBody.innerHTML = '';
  }
  if (inventoryBody) {
  ...
  if (!sheet.migratedAssets) {
    const validKeys = new Set(rows.map(item => item.key));
    Object.entries(sheet.assets || {}).forEach(([key, value]) => {
      if (!validKeys.has(key) && value && typeof value === 'object') {
        const baseCategory = key.split('_')[0];
        const definition = SECRET_ASSET_CATEGORIES.find(cat => cat.category === baseCategory);
        sheet.extraAssets.push({
          id: generateSecretEntryId('asset'),
          category: value.category || (definition ? definition.category : baseCategory) || 'other',
          label: value.label || (definition ? definition.label : 'Дополнительный актив'),
          name: value.name || '',
          price: Number(value.price) || 0,
          quantity: value.allowQuantity === false ? 1 : (Number(value.quantity) || 0),
          income: Number(value.income) || 0,
          allowQuantity: value.allowQuantity === false ? false : true,
          confirmed: !!value.confirmed,
          appliedIncome: Number(value.appliedIncome) || 0,
          trackRemaining: !!value.trackRemaining,
          remaining: Number(value.remaining) || 0
        });
        delete sheet.assets[key];
      }
    });
    sheet.migratedAssets = true;
  }

  gameData.secretData.sheets[round] = sheet;

  const appendRow = (container, source) => {
    if (!container) return;

    const isExtra = source.type === 'extra';
    const item = source.item;
    const entry = source.entry;
    const label = isExtra ? (entry.label || 'Дополнительный актив') : item.label;
    const category = isExtra ? (entry.category || 'other') : (item.category || 'other');
    const family = getSecretAssetFamily(category);
    const allowQuantity = isExtra ? (entry.allowQuantity === false ? false : true) : (item.allowQuantity !== false);
    const allowName = isExtra ? true : item.allowName !== false;
    const trackRemaining = isExtra ? !!entry.trackRemaining : !!item.trackRemaining;
    ...
    const familyStyle = SECRET_ASSET_FAMILY_STYLES[family];
    if (familyStyle) {
      row.style.setProperty('--row-accent', familyStyle.accent);
      row.style.setProperty('--row-fill', familyStyle.fill);
    }
    if (values.confirmed) {
    ...
    labelHeader.className = 'asset-label-header';
    const labelTitle = document.createElement('span');
    labelTitle.className = 'asset-label-title';
    ...
    labelWrapper.appendChild(labelHeader);
    ...
    if (isSalary) {
      priceCell.innerHTML = '<span class="placeholder">—</span>';
    } else {
      const priceInput = document.createElement('input');
      priceInput.type = 'number';
      priceInput.min = '0';
      priceInput.step = '100';
      priceInput.value = values.price || 0;
      priceInput.dataset.secretRound = round;
      priceInput.dataset.secretField = 'price';
      if (isExtra) {
        priceInput.dataset.secretSection = 'extra-assets';
        priceInput.dataset.secretEntry = entry.id;
      } else {
        priceInput.dataset.secretSection = 'assets';
        priceInput.dataset.secretKey = item.key;
      }
      priceCell.appendChild(priceInput);
    }

    const quantityCell = document.createElement('td');
    if (!isSalary && allowQuantity) {
      const quantityInput = document.createElement('input');
      quantityInput.type = 'number';
      quantityInput.min = '0';
      quantityInput.step = '1';
      quantityInput.value = values.quantity || 0;
      quantityInput.dataset.secretRound = round;
      quantityInput.dataset.secretField = 'quantity';
      if (isExtra) {
        quantityInput.dataset.secretSection = 'extra-assets';
        quantityInput.dataset.secretEntry = entry.id;
      } else {
        quantityInput.dataset.secretSection = 'assets';
        quantityInput.dataset.secretKey = item.key;
      }
    ...
    if (isSalary) {
      ...
      salaryValue.className = 'asset-readonly';
      salaryValue.textContent = values.income ? `₽${formatRub(values.income)}` : '—';
      measureCell.appendChild(salaryValue);
    } else if (trackRemaining) {
      const remainingInput = document.createElement('input');
      remainingInput.type = 'number';
      remainingInput.min = '0';
      remainingInput.step = '1';
      remainingInput.value = values.remaining || 0;
      remainingInput.dataset.secretRound = round;
      remainingInput.dataset.secretField = 'remaining';
      if (isExtra) {
        remainingInput.dataset.secretSection = 'extra-assets';
        remainingInput.dataset.secretEntry = entry.id;
      } else {
        remainingInput.dataset.secretSection = 'assets';
        remainingInput.dataset.secretKey = item.key;
      }
      measureCell.appendChild(remainingInput);
    } else {
      const incomeInput = document.createElement('input');
      incomeInput.type = 'number';
      incomeInput.min = '0';
      incomeInput.step = '100';
      incomeInput.value = values.income || 0;
      incomeInput.dataset.secretRound = round;
      incomeInput.dataset.secretField = 'income';
      if (isExtra) {
        incomeInput.dataset.secretSection = 'extra-assets';
        incomeInput.dataset.secretEntry = entry.id;
      } else {
        incomeInput.dataset.secretSection = 'assets';
        incomeInput.dataset.secretKey = item.key;
      }
      measureCell.appendChild(incomeInput);
    }

    const actionsCell = document.createElement('td');
    actionsCell.className = 'asset-actions-cell';

    if (isSalary) {
      const status = document.createElement('span');
      status.className = 'asset-status salary';
      status.textContent = 'Профессия';
      actionsCell.appendChild(status);
      if (Number(values.appliedIncome) > 0) {
        const incomeBadge = document.createElement('span');
        incomeBadge.className = 'asset-income-badge';
        incomeBadge.textContent = `+₽${formatRub(Number(values.appliedIncome) || 0)}`;
        actionsCell.appendChild(incomeBadge);
      }
    } else {
      const confirmBtn = document.createElement('button');
      confirmBtn.type = 'button';
      confirmBtn.className = values.confirmed ? 'asset-confirm-btn confirmed' : 'asset-confirm-btn';
      confirmBtn.textContent = values.confirmed ? 'Обновить' : 'Подтвердить';
      confirmBtn.onclick = isExtra ? () => confirmSecretExtraAsset(round, entry.id) : () => confirmSecretAsset(round, item.key);
      actionsCell.appendChild(confirmBtn);

      const sellBtn = document.createElement('button');
      sellBtn.type = 'button';
      sellBtn.className = 'asset-sell-btn';
      sellBtn.textContent = 'Продать';
      sellBtn.onclick = isExtra
        ? () => secretSellAsset(round, { entryId: entry.id, extra: true, label })
        : () => secretSellAsset(round, { key: item.key, extra: false, label });
      actionsCell.appendChild(sellBtn);

      if (values.confirmed && Number(values.appliedIncome) > 0 && !trackRemaining) {
        const incomeBadge = document.createElement('span');
        incomeBadge.className = 'asset-income-badge';
        incomeBadge.textContent = `+₽${formatRub(Number(values.appliedIncome) || 0)}`;
        actionsCell.appendChild(incomeBadge);
  ...
  rows.forEach(item => {
    ...
    const defaults = {
      name: '',
      price: 0,
      quantity: allowQuantity ? 0 : 1,
      income: 0,
      confirmed: false,
      appliedIncome: 0,
      remaining: 0,
      trackRemaining: !!item.trackRemaining
    };
    const stored = sheet.assets[item.key] || {};
    const values = { ...defaults, ...stored };
    values.trackRemaining = !!(item.trackRemaining || values.trackRemaining);
    const target = item.category === 'inventory' ? inventoryBody : mainBody;
    appendRow(target, { type: 'base', item, values });
  });
  ...
  (sheet.extraAssets || []).forEach(entry => {
    ...
    const target = entry.category === 'inventory' ? inventoryBody : mainBody;
    appendRow(target, { type: 'extra', entry, values });
  });

  if (inventoryBody) {
    const card = document.getElementById(`secretRound${round}InventoryCard`);
    if (card) {
      card.style.display = inventoryBody.children.length ? '' : 'none';
    }
  }
}

function confirmSecretAsset(round, key) {
  const sheet = ensureSecretSheet(round);
  if (!sheet || !sheet.assets) return;
  const asset = sheet.assets[key];
  if (!asset) {
    showModal('Ошибка', 'Заполните данные по активу перед подтверждением.');
    return;
  }

  const meta = getSecretAssetDefinition(round, key);
  const label = meta ? meta.label : asset.label || 'Актив';
  const allowQuantity = meta ? meta.allowQuantity !== false : asset.allowQuantity !== false;
  const trackRemaining = meta ? !!meta.trackRemaining : !!asset.trackRemaining;
  const name = (asset.name || '').trim();
  const price = Number(asset.price) || 0;
  const income = Number(asset.income) || 0;
  let quantity = allowQuantity ? Number(asset.quantity) || 0 : 1;
  const remaining = trackRemaining ? Number(asset.remaining) || 0 : 0;
  if (allowQuantity && quantity <= 0) {
    quantity = 1;
  }

  if (!trackRemaining && !price && !income) {
    showModal('Недостаточно данных', 'Укажите стоимость или доход, чтобы подтвердить актив.');
    return;
  }

  if (trackRemaining && price <= 0 && quantity <= 0 && remaining <= 0) {
    showModal('Недостаточно данных', 'Укажите цену, количество или остатки товара, чтобы подтвердить актив.');
    return;
  }

  const appliedIncome = trackRemaining ? 0 : income * (allowQuantity ? quantity : 1);
  const summary = [`Актив: ${label}`];
  if (name) summary.push(`Наименование: ${name}`);
  if (price) summary.push(`Цена: ₽${formatRub(price)}`);
  if (allowQuantity) summary.push(`Количество: ${quantity}`);
  if (trackRemaining) {
    summary.push(`Остаток: ${remaining}`);
  } else if (income) {
    summary.push(`Доход: ₽${formatRub(income)}${allowQuantity ? ' за единицу' : ''}`);
  }
  if (appliedIncome) summary.push(`Пассивный доход: ₽${formatRub(appliedIncome)}`);
  summary.push('', 'Подтвердить покупку актива?');
  ...
  asset.price = price;
  asset.income = income;
  asset.quantity = allowQuantity ? quantity : 1;
  asset.confirmed = true;
  asset.appliedIncome = appliedIncome;
  asset.allowQuantity = allowQuantity;
  asset.category = asset.category || (meta ? meta.category : 'other');
  asset.label = asset.label || label;
  asset.trackRemaining = trackRemaining;
  asset.remaining = trackRemaining ? remaining : 0;
  ...
  const incomeNote = appliedIncome ? ` +₽${formatRub(appliedIncome)} к пассивному доходу.` : (trackRemaining ? ' Остатки зафиксированы.' : '.');
  showModal('Актив подтверждён', `${label} зафиксирован${incomeNote}`);
}

function resetSecretAsset(round, key) {
  const sheet = ensureSecretSheet(round);
  if (!sheet || !sheet.assets) return;
  const asset = sheet.assets[key];
  if (!asset) return;
  ...
  const meta = getSecretAssetDefinition(round, key);
  const label = meta ? meta.label : asset.label || 'Актив';
  if (!confirm(`Очистить данные по активу «${label}»?`)) {
    return;
  }

  asset.price = 0;
  asset.income = 0;
  asset.appliedIncome = 0;
  asset.remaining = 0;
  asset.confirmed = false;
  if (asset.allowQuantity !== false) {
    asset.quantity = 0;
  } else {
    asset.quantity = 1;
  }
  asset.trackRemaining = meta ? !!meta.trackRemaining : !!asset.trackRemaining;
...
function confirmSecretExtraAsset(round, entryId) {
  const sheet = ensureSecretSheet(round);
  if (!sheet) return;
  sheet.extraAssets = Array.isArray(sheet.extraAssets) ? sheet.extraAssets : [];
  ...
  const label = entry.label || 'Дополнительный актив';
  const name = (entry.name || '').trim();
  const price = Number(entry.price) || 0;
  const income = Number(entry.income) || 0;
  const allowQuantity = entry.allowQuantity === false ? false : true;
  let quantity = allowQuantity ? Number(entry.quantity) || 0 : 1;
  const trackRemaining = !!entry.trackRemaining;
  const remaining = trackRemaining ? Number(entry.remaining) || 0 : 0;
  if (allowQuantity && quantity <= 0) {
    quantity = 1;
  }

  if (!trackRemaining && !price && !income) {
    showModal('Недостаточно данных', 'Добавьте стоимость или доход, чтобы подтвердить актив.');
    return;
  }
  ...
  entry.price = price;
  entry.income = income;
  entry.quantity = allowQuantity ? quantity : 1;
  entry.allowQuantity = allowQuantity;
  entry.confirmed = true;
  entry.appliedIncome = appliedIncome;
  entry.trackRemaining = trackRemaining;
  entry.remaining = trackRemaining ? remaining : 0;
...
function resetSecretExtraAsset(round, entryId) {
  const sheet = ensureSecretSheet(round);
  if (!sheet) return;
  sheet.extraAssets = Array.isArray(sheet.extraAssets) ? sheet.extraAssets : [];
  const entry = sheet.extraAssets.find(item => item.id === entryId);
  ...
  entry.price = 0;
  entry.income = 0;
  entry.appliedIncome = 0;
  entry.remaining = 0;
  entry.confirmed = false;
  if (entry.allowQuantity === false) {
    entry.quantity = 1;
  } else {
    entry.quantity = 0;
  }
  entry.trackRemaining = !!entry.trackRemaining;
...
function secretSellAsset(round, payload = {}) {
  const sheet = ensureSecretSheet(round);
  if (!sheet) return;

  if (typeof window === 'undefined' || typeof window.prompt !== 'function') {
    showModal('Продажа актива', 'Продажа доступна только в браузере.');
    return;
  }

  const isExtra = !!payload.extra;
  let asset;
  let label;
  let category;
  let allowQuantity = true;
  let quantity = 1;
  let price = 0;
  let trackRemaining = false;
  let meta = null;

  if (isExtra) {
    asset = (sheet.extraAssets || []).find(entry => entry.id === payload.entryId);
    if (!asset) {
      showModal('Продажа актива', 'Не удалось найти актив для продажи.');
      return;
    }
    label = payload.label || asset.label || 'Актив';
    category = asset.category || 'other';
    allowQuantity = asset.allowQuantity === false ? false : true;
    quantity = allowQuantity ? Math.max(0, Number(asset.quantity) || 0) : 1;
    price = Number(asset.price) || 0;
    trackRemaining = !!asset.trackRemaining;
  } else {
    meta = getSecretAssetDefinition(round, payload.key);
    asset = sheet.assets[payload.key];
    if (!asset) {
      showModal('Продажа актива', 'Не удалось найти актив для продажи.');
      return;
    }
    label = payload.label || meta?.label || asset.label || 'Актив';
    category = asset.category || meta?.category || 'other';
    allowQuantity = meta ? meta.allowQuantity !== false : asset.allowQuantity !== false;
    quantity = allowQuantity ? Math.max(0, Number(asset.quantity) || 0) : 1;
    price = Number(asset.price) || 0;
    trackRemaining = !!asset.trackRemaining;
  }
  ...
  if (!asset.confirmed) {
    showModal('Продажа актива', 'Сначала подтвердите актив перед продажей.');
    return;
  }

  const baseTotal = allowQuantity ? price * (quantity || 0) : price;

  const promptAmount = (message, defaultValue = '') => {
    const raw = window.prompt(message, defaultValue ? String(defaultValue) : '');
    if (raw === null) {
      return null;
    }
    ...
    if (!Number.isFinite(value) || value <= 0) {
      showModal('Продажа актива', 'Введите корректную сумму.');
      return null;
    }
    return value;
  };

  const promptPercent = message => {
    ...
    if (!Number.isFinite(value)) {
      showModal('Продажа актива', 'Введите корректное значение процента.');
      return null;
    }
    return value;
  };

  const promptTurns = message => {
    ...
    const value = Number(raw.replace(',', '.'));
    if (!Number.isFinite(value) || value <= 0) {
      showModal('Продажа актива', 'Введите корректное количество ходов.');
      return null;
    }
  ...
  switch (category) {
    case 'realEstate': {
      if (baseTotal <= 0) {
        showModal('Продажа недвижимости', 'Укажите стоимость недвижимости перед продажей.');
        return;
      }
      const marketPercent = promptPercent('На сколько процентов изменилась цена недвижимости на рынке сегодня?');
      if (marketPercent === null) return;
      saleAmount = baseTotal * (1 + marketPercent / 100);
      note = `Продажа недвижимости (${marketPercent >= 0 ? '+' : ''}${marketPercent}%)`;
      break;
    }
    case 'construction': {
      if (baseTotal <= 0) {
        showModal('Продажа строительства', 'Укажите стоимость строительства перед продажей.');
        return;
      }
      const buildPercent = promptPercent('На сколько процентов выросла стоимость после строительства?');
      if (buildPercent === null) return;
      const marketPercent = promptPercent('На сколько процентов изменилась цена на рынке сегодня?');
      if (marketPercent === null) return;
      saleAmount = baseTotal * (1 + buildPercent / 100) * (1 + marketPercent / 100);
      note = `Продажа строительства (${buildPercent >= 0 ? '+' : ''}${buildPercent}% / ${marketPercent >= 0 ? '+' : ''}${marketPercent}%)`;
      break;
    }
    case 'land': {
      const amount = promptAmount('Введите сумму продажи земельного участка', baseTotal || '');
      if (amount === null) return;
      saleAmount = amount;
      note = 'Продажа земельного участка';
      break;
    }
    case 'securities': {
      if (quantity <= 0) {
        showModal('Продажа актива', 'Укажите количество бумаг перед продажей.');
        return;
      }
      const unitPrice = promptAmount('Введите цену продажи за одну ценную бумагу');
      if (unitPrice === null) return;
      saleAmount = unitPrice * quantity;
      note = 'Продажа ценных бумаг';
      break;
    }
    case 'funds': {
      if (quantity <= 0) {
        showModal('Продажа актива', 'Укажите количество паёв перед продажей.');
        return;
      }
      const fundPrice = promptAmount('Введите цену продажи одного пая');
      if (fundPrice === null) return;
      saleAmount = fundPrice * quantity;
      note = 'Продажа паевых фондов';
      break;
    }
    case 'business': {
      const amount = promptAmount('Введите сумму продажи доли в бизнесе', baseTotal || '');
      if (amount === null) return;
      saleAmount = amount;
      note = 'Продажа доли в бизнесе';
      break;
    }
    case 'inventory': {
      const amount = promptAmount('Введите сумму продажи товара');
      if (amount === null) return;
      saleAmount = amount;
      note = 'Продажа товара';
      break;
    ...
    case 'deposits': {
      if (baseTotal <= 0) {
        showModal('Закрытие вклада', 'Укажите сумму вклада перед закрытием.');
        return;
      }
      const rate = promptPercent('Под какой процент размещался вклад?');
      if (rate === null) return;
      const turns = promptTurns('Сколько ходов длился вклад?');
      if (turns === null) return;
      saleAmount = baseTotal * (1 + rate / 100);
      note = `Закрытие вклада (${turns} ход${turns % 10 === 1 && turns % 100 !== 11 ? '' : turns % 10 >= 2 && turns % 10 <= 4 && (turns % 100 < 10 || turns % 100 >= 20) ? 'а' : 'ов'})`;
      break;
    }
    case 'privateLoans': {
      const amount = promptAmount('Введите сумму возврата по частному займу', baseTotal || '');
      if (amount === null) return;
      saleAmount = amount;
      note = 'Возврат частного займа';
      break;
    }
    case 'gold': {
      const amount = promptAmount('Введите сумму продажи золотых изделий', baseTotal || '');
      if (amount === null) return;
      saleAmount = amount;
      note = 'Продажа золотых изделий';
      break;
    }
    default: {
      const amount = promptAmount('Введите сумму продажи актива', baseTotal || '');
      if (amount === null) return;
      saleAmount = amount;
      note = `Продажа актива «${label}»`;
      break;
    }
  }

  if (!Number.isFinite(saleAmount) || saleAmount <= 0) {
    showModal('Продажа актива', 'Сумма продажи должна быть больше нуля.');
    return;
  }

  const summary = [`Актив: ${label}`, `Сумма продажи: ₽${formatRub(saleAmount)}`];
  if (note && !note.toLowerCase().startsWith('продажа актива')) {
    summary.push(note);
  }
  summary.push('', 'Провести продажу?');
  if (typeof window.confirm === 'function' && !window.confirm(summary.join('\n'))) {
    return;
  }
  ...
  if (!isExtra && payload.key) {
    sheet.assets[payload.key] = asset;
  }
  ...
  applySecretLedgerEntry(round, {
    type: saleAmount >= 0 ? 'income' : 'expense',
    amount: Math.abs(saleAmount),
    note: note || `Продажа актива «${label}»`
  });

  checkSecretUnlocks();
  showModal('Продажа завершена', `${label} продан за ₽${formatRub(saleAmount)}. Средства добавлены в кошелёк.`);
}
...
function renderSecretNetwork(round, sheet) {
  ...
  products.forEach(product => {
    ...
    const multiplierSpan = document.getElementById(`secretRound${round}${product.charAt(0).toUpperCase() + product.slice(1)}Multiplier`);
    ...
    if (totalSpan) {
      ...
      const base = (Number(data.income) || 0) * (Number(data.count) || 0);
      const total = Number(data.applied) || base * multiplier;
      totalSpan.textContent = `₽${formatRub(total)}`;
      if (multiplierSpan) {
        multiplierSpan.textContent = multiplier > 1 ? `×${multiplier}` : '';
      }
...
function secretClearMarketing(round, product) {
  if (!SECRET_MARKETING_LABELS[product]) {
    return;
  }

  const sheet = ensureSecretSheet(round);
  if (!sheet) return;
  sheet.marketing = sheet.marketing || {};
  sheet.marketing[product] = sheet.marketing[product] || { income: 0, count: 0, multiplier: 1, applied: 0 };

  const label = SECRET_MARKETING_LABELS[product];
  if (!confirm(`Очистить данные по направлению «${label}»?`)) {
    return;
  }

  sheet.marketing[product].income = 0;
  sheet.marketing[product].count = 0;
  sheet.marketing[product].multiplier = 1;
  sheet.marketing[product].applied = 0;
...
function secretBoostMarketing(round, multiplier) {
  const allowedMultiplier = Number(multiplier);
  if (![2, 3].includes(allowedMultiplier)) {
    return;
  }
  ...
  const candidates = ['packages', 'vitamins'].filter(product => {
    const data = sheet.marketing[product] || { income: 0, count: 0 };
    const base = (Number(data.income) || 0) * (Number(data.count) || 0);
    return base > 0;
  });
  ...
  if (!candidates.length) {
    showModal('Нет данных', 'Заполните доход и количество в сетевом маркетинге, чтобы умножить результат.');
    return;
  }

  let selected = candidates[0];
  if (candidates.length > 1) {
    const options = candidates.map((product, index) => `${index + 1}. ${SECRET_MARKETING_LABELS[product]}`).join('\n');
    const answer = prompt(`Выберите направление для умножения дохода:\n${options}`);
    if (answer === null) {
      return;
    }
    const index = Number(answer.trim());
    if (!index || index < 1 || index > candidates.length) {
      showModal('Ошибка', 'Введите номер направления из списка.');
      return;
    }
    selected = candidates[index - 1];
  }

  sheet.marketing[selected] = sheet.marketing[selected] || { income: 0, count: 0, multiplier: 1, applied: 0 };
  const data = sheet.marketing[selected];
  if ((Number(data.income) || 0) <= 0 || (Number(data.count) || 0) <= 0) {
    showModal('Нет данных', `Заполните доход и количество для «${SECRET_MARKETING_LABELS[selected]}».`);
    return;
  }

  data.multiplier = allowedMultiplier;
  showModal('Доход увеличен', `${SECRET_MARKETING_LABELS[selected]} теперь умножается ×${allowedMultiplier}.`);
...
function renderSecretTotals(round, sheet) {
  ...
  if (incomeInput && incomeInput !== document.activeElement) {
    const incomeValue = typeof sheet.totals?.income === 'number' ? sheet.totals.income : '';
    incomeInput.value = incomeValue;
  }
  if (passiveInput && passiveInput !== document.activeElement) {
    const passiveValue = typeof sheet.totals?.passive === 'number' ? sheet.totals.passive : '';
    passiveInput.value = passiveValue;
  }
  if (regularInput) {
...
function secretAddAssetRow(round) {
  ...
  const sheet = ensureSecretSheet(round);
  sheet.extraAssets = Array.isArray(sheet.extraAssets) ? sheet.extraAssets : [];
  const baseDefinition = SECRET_ASSET_CATEGORIES.find(item => item.category === category);
  const allowQuantity = baseDefinition ? baseDefinition.allowQuantity !== false : true;
  const trackRemaining = !!(baseDefinition && baseDefinition.trackRemaining);
  sheet.extraAssets.push({
    id: generateSecretEntryId('asset'),
    category,
...
function secretLedgerApply(round, type) {
  ...
  if (!note) {
    showModal('Учётная книга', 'Добавьте комментарий к записи.');
    return;
  }
...
function secretTriggerWalletAction(action) {
  const secret = ensureSecretState(gameData.secretData);
  const activeRound = secret.round || 1;

  if (action === 'salary') {
    secretLedgerPaySalary(activeRound);
    return;
  }

  if (action === 'income' || action === 'expense') {
    secretLedgerApply(activeRound, action);
  }
}
...
function aggregateSecretIncome(round, sheet) {
  if (!sheet) {
    return { passiveIncome: 0, salaryIncome: 0, marketingIncome: 0, totalIncome: 0, regular: 0 };
  }

  let passiveIncome = 0;
  let salaryIncome = 0;
  const assetEntries = sheet.assets && typeof sheet.assets === 'object' ? sheet.assets : {};

  Object.entries(assetEntries).forEach(([key, asset]) => {
    if (!asset) return;
    const category = asset.category || key;
    const allowQuantity = asset.allowQuantity === false ? false : true;
    const quantity = allowQuantity ? Math.max(0, Number(asset.quantity) || 0) : 1;
    const income = Number(asset.income) || 0;
    const trackRemaining = !!asset.trackRemaining;
    const applied = asset.confirmed && !trackRemaining ? income * (allowQuantity ? quantity : 1) : 0;
    asset.appliedIncome = asset.confirmed ? applied : 0;

    if (asset.confirmed && applied > 0) {
      if (category === 'salary') {
        salaryIncome += applied;
      } else {
        passiveIncome += applied;
      }
    }
  });
  ...
  if (Array.isArray(sheet.extraAssets)) {
    sheet.extraAssets.forEach(entry => {
      const allowQuantity = entry.allowQuantity === false ? false : true;
      const quantity = allowQuantity ? Math.max(0, Number(entry.quantity) || 0) : 1;
      const income = Number(entry.income) || 0;
      const trackRemaining = !!entry.trackRemaining;
      const applied = entry.confirmed && !trackRemaining ? income * (allowQuantity ? quantity : 1) : 0;
      entry.appliedIncome = entry.confirmed ? applied : 0;
      if (entry.confirmed && applied > 0) {
        passiveIncome += applied;
      }
    });
  }

  sheet.marketing = sheet.marketing || { packages: { income: 0, count: 0, multiplier: 1 }, vitamins: { income: 0, count: 0, multiplier: 1 } };
  const marketingProducts = ['packages', 'vitamins'];
  let marketingIncome = 0;

  marketingProducts.forEach(product => {
    const data = sheet.marketing[product] || { income: 0, count: 0, multiplier: 1 };
    const multiplier = Math.max(1, Number(data.multiplier) || 1);
    const base = (Number(data.income) || 0) * (Number(data.count) || 0);
    const applied = base * multiplier;
    data.multiplier = multiplier;
    data.applied = applied;
    sheet.marketing[product] = data;
    marketingIncome += applied;
  });

  const baseRegular = Number(sheet.totals?.baseRegular) || 0;
  const totalIncome = passiveIncome + salaryIncome + marketingIncome;
  const regular = baseRegular + passiveIncome + marketingIncome;

  sheet.totals = sheet.totals || { income: 0, passive: 0, regular: baseRegular, baseRegular };
  sheet.totals.passive = Math.round(passiveIncome);
  sheet.totals.income = Math.round(totalIncome);
  sheet.totals.regular = Math.round(regular);
  sheet.totals.salaryIncome = Math.round(salaryIncome);
  sheet.totals.marketingIncome = Math.round(marketingIncome);

  return { passiveIncome, salaryIncome, marketingIncome, totalIncome, regular };
}
...
function requestSecretDreamConfirmation(value) {
  const secret = ensureSecretState(gameData.secretData);
  const trimmed = (value || '').trim();

  if (!trimmed) {
    secret.playerInfo.dream = '';
    secret.playerInfo.dreamConfirmed = false;
    secret.goals = secret.goals || createDefaultSecretGoals();
    secret.goals.dreamAchieved = false;
  ...
  const confirmed = confirm('Вы уверены, что мечта вам откликается?');
  if (confirmed) {
    secret.playerInfo.dream = trimmed;
    secret.playerInfo.dreamConfirmed = true;
    showModal('Мечта зафиксирована', 'Вы подтвердили свою мечту. Она будет сопровождать вас до финальной победы.');
  } else {
    secret.playerInfo.dreamConfirmed = false;
    showModal('Мечта не зафиксирована', 'Скорректируйте мечту и подтвердите её ещё раз, когда будете готовы.');
  }
...
function updateSecretDreamVisuals() {
  const secret = ensureSecretState(gameData.secretData);
  const dreamFilled = !!(secret.playerInfo.dream && secret.playerInfo.dream.trim());
  const dreamConfirmed = !!secret.playerInfo.dreamConfirmed;

  document.querySelectorAll('.dream-field').forEach(field => {
    field.classList.toggle('filled', dreamFilled);
    field.classList.toggle('confirmed', dreamConfirmed);
  });
}

function updateSecretGoalsUI() {
  const secret = ensureSecretState(gameData.secretData);
  const goals = secret.goals || createDefaultSecretGoals();
  if (!secret.goals) {
    secret.goals = goals;
    gameData.secretData = secret;
  }
  const unlockedRound3 = secret.unlockedRounds.includes(3);
  const panel = document.getElementById('secretGoalsPanel');
  if (panel) {
    panel.style.display = unlockedRound3 ? '' : 'none';
  }

  const totals = calculateSecretRoundTotals(3);
  const passiveIncome = totals.passiveIncome;
  const target = Number(goals.passiveTarget) || 2000000;
  const passiveText = document.getElementById('secretPassiveGoalProgress');
  if (passiveText) {
    passiveText.textContent = `₽${formatRub(passiveIncome)} / ₽${formatRub(target)}`;
  }

  const passiveBtn = document.getElementById('secretPassiveGoalButton');
  if (passiveBtn) {
    passiveBtn.disabled = goals.passiveAchieved || passiveIncome < target;
  }

  const passiveCard = document.getElementById('secretPassiveGoalCard');
  if (passiveCard) {
    passiveCard.classList.toggle('completed', !!goals.passiveAchieved);
  }

  const dreamText = secret.playerInfo.dream || '—';
  const dreamConfirmed = !!secret.playerInfo.dreamConfirmed;
  const dreamDisplay = document.getElementById('secretDreamGoalText');
  if (dreamDisplay) {
    dreamDisplay.textContent = dreamText || '—';
  }

  const dreamBtn = document.getElementById('secretDreamGoalButton');
  if (dreamBtn) {
    dreamBtn.disabled = goals.dreamAchieved || !dreamConfirmed;
  }

  const dreamCard = document.getElementById('secretDreamGoalCard');
  if (dreamCard) {
    dreamCard.classList.toggle('confirmed', dreamConfirmed);
    dreamCard.classList.toggle('completed', !!goals.dreamAchieved);
  }

  const notice = document.getElementById('secretGoalNotice');
  const noticeMessage = document.getElementById('secretGoalNoticeMessage');
  const achievedCount = (goals.passiveAchieved ? 1 : 0) + (goals.dreamAchieved ? 1 : 0);
  if (notice) {
    if (unlockedRound3 && achievedCount === 1) {
      notice.classList.add('active');
      if (noticeMessage) {
        noticeMessage.textContent = goals.passiveAchieved
          ? 'Пассивный доход достиг цели. Осталось исполнить мечту, чтобы завершить игру.'
          : 'Мечта подтверждена! Осталось довести пассивный доход до цели для полной победы.';
      }
    } else {
      notice.classList.remove('active');
    }
  ...
  const banner = document.getElementById('secretVictoryBanner');
  if (banner) {
    const victoryReached = goals.victoryShown || (goals.passiveAchieved && goals.dreamAchieved);
    banner.classList.toggle('active', victoryReached);
  }

  const reportButton = document.getElementById('secretReportButton');
  if (reportButton) {
    reportButton.disabled = !goals.reportUnlocked;
    reportButton.classList.toggle('ready', !!goals.reportUnlocked);
  }
}

function confirmSecretPassiveGoal() {
  const secret = ensureSecretState(gameData.secretData);
  const goals = secret.goals || createDefaultSecretGoals();
  if (goals.passiveAchieved) {
    showModal('Цель уже достигнута', 'Вы уже зафиксировали рост пассивного дохода. Осталось выполнить вторую задачу.');
    return;
  }

  const totals = calculateSecretRoundTotals(3);
  if (totals.passiveIncome < goals.passiveTarget) {
    showModal('Цель ещё не достигнута', 'Пассивный доход пока ниже целевого значения. Продолжайте наращивать активы.');
    return;
  }

  goals.passiveAchieved = true;
  gameData.secretData.goals = goals;
  showModal('Цель достигнута', 'Пассивный доход соответствует плану! Осталось исполнить мечту, чтобы выиграть игру.');
  handleSecretGoalProgress();
}

function confirmSecretDreamGoal() {
  const secret = ensureSecretState(gameData.secretData);
  const goals = secret.goals || createDefaultSecretGoals();
  if (goals.dreamAchieved) {
    showModal('Мечта уже подтверждена', 'Вы уже отметили исполнение мечты. Перейдите к росту пассивного дохода.');
    return;
  }

  if (!secret.playerInfo.dreamConfirmed || !(secret.playerInfo.dream || '').trim()) {
    showModal('Мечта не подтверждена', 'Сначала зафиксируйте мечту и убедитесь, что она вам откликается.');
    return;
  }

  if (!confirm('Подтвердить, что мечта достигнута?')) {
    return;
  }

  goals.dreamAchieved = true;
  gameData.secretData.goals = goals;
  showModal('Мечта достигнута', 'Поздравляем! Мечта реализована. Теперь доведите пассивный доход до цели.');
  handleSecretGoalProgress();
}

function handleSecretGoalProgress() {
  const secret = ensureSecretState(gameData.secretData);
  const goals = secret.goals || createDefaultSecretGoals();
  gameData.secretData.goals = goals;

  if (goals.passiveAchieved && goals.dreamAchieved) {
    celebrateSecretVictory();
  } else {
    updateSecretGoalsUI();
    saveGameData();
  }
...
function celebrateSecretVictory() {
  const secret = ensureSecretState(gameData.secretData);
  const goals = secret.goals || createDefaultSecretGoals();
  if (goals.victoryShown) {
    goals.victoryShown = true;
    gameData.secretData.goals = goals;
    updateSecretGoalsUI();
    saveGameData();
    return;
  ...
  goals.victoryShown = true;
  goals.reportUnlocked = false;
  gameData.secretData.goals = goals;

  const banner = document.getElementById('secretVictoryBanner');
  if (banner) {
    banner.classList.add('active', 'celebrate');
    setTimeout(() => banner.classList.remove('celebrate'), 2500);
  }

  if (secretVictoryTimer) {
    clearTimeout(secretVictoryTimer);
  }
  secretVictoryTimer = setTimeout(() => {
    const latest = ensureSecretState(gameData.secretData);
    latest.goals = latest.goals || createDefaultSecretGoals();
    latest.goals.reportUnlocked = true;
    gameData.secretData = latest;
    updateSecretGoalsUI();
    saveGameData();
  }, 3000);

  showModal('Поздравляем!', 'Обе цели выполнены! Насладитесь моментом и подготовьтесь к финальному отчету.');
  updateSecretGoalsUI();
  saveGameData();
}

function showSecretReport() {
  const secret = ensureSecretState(gameData.secretData);
  const goals = secret.goals || createDefaultSecretGoals();
  if (!goals.reportUnlocked) {
    showModal('Отчет ещё не готов', 'Подождите несколько секунд после победы, чтобы перейти к финальному анализу.');
    return;
  }
...
function handleSecretFieldInput(event) {
  ...
  if (section === 'assets') {
    ...
    sheet.assets[key].confirmed = false;
    sheet.assets[key].appliedIncome = 0;

    const row = target.closest('tr.secret-asset-row');
    if (row) {
      row.classList.remove('confirmed');
      const badge = row.querySelector('.asset-income-badge');
      if (badge) {
        badge.remove();
      }
      const status = row.querySelector('.asset-status');
      if (status && !status.classList.contains('salary')) {
        status.classList.remove('confirmed');
        status.textContent = 'Черновик';
      }
      const confirmBtn = row.querySelector('.asset-confirm-btn');
      if (confirmBtn) {
    ...
    if (field === 'player') {
      ...
      const secret = ensureSecretState(gameData.secretData);
      secret.playerInfo.dream = target.value;
      secret.playerInfo.dreamConfirmed = false;
      secret.goals = secret.goals || createDefaultSecretGoals();
      secret.goals.dreamAchieved = false;
      gameData.secretData = secret;
      ...
      if (event.type === 'change') {
        requestSecretDreamConfirmation(target.value);
        return;
      }
    }
  }
...
function unlockSecretRound(round) {
  ...
  ensureSecretSheet(round);
  if (round === 2) {
    inheritSecretAssets(1, 2, ['realEstate', 'land', 'construction', 'securities', 'funds', 'business']);
    const sheet2 = ensureSecretSheet(2);
  ...
  if (round === 3) {
    inheritSecretAssets(2, 3, ['realEstate', 'land', 'construction', 'securities', 'funds', 'business', 'inventory']);
    ensureSecretSheet(3);
  }
